...
traindata_path = cfg["traindata_path"]
...
for epoch in range(num_epochs):
    training_loss = []
    val_loss = []
    for step, (input_batch, target_batch) in enumerate(training_loader):        
    ...
    with torch.no_grad():
        model.eval()
        for _, (input_batch, target_batch) in enumerate(validation_loader):            
            input_batch = input_batch.cuda()
            target_batch = target_batch.cuda()
            outputs = model(input_batch, target_batch)
            cur_loss = criterion(outputs, target_batch)
